...
describe('WASM Loader', () => {
  ...
  it('should successfully load and return the WASM module', async () => {
    ...
    const mockStringifyFromDimens = jest.fn().mockImplementation(
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('WASM module loaded successfully');
  });
  ...
  it('should handle failure to get pointer from WASM module', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to get pointer from WASM module');
  });
  ...
  it('should handle failure to initialize WASM module', async () => {
    ...
    // Verify error handling
    expect(result).toBeNull();
    expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to initialize WASM module');
  });
