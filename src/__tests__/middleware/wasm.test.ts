...
describe('WASM Middleware', () => {
  ...
  it('should attach WASM instance to request when loaded successfully', async () => {
    ...
    (mazeService.getWasmInstance as jest.Mock).mockResolvedValue(mockWasmInstance);
    
    // Call middleware
    await wasmMiddleware(mockRequest as AuthRequest, mockResponse as Response, mockNext);
  ...
  it('should proceed without WASM instance when loading fails', async () => {
    ...
    expect(mockNext).toHaveBeenCalled();
    expect(consoleWarnSpy).toHaveBeenCalledWith('WASM middleware: Failed to load WASM module');
  });
  ...
  it('should handle exceptions and proceed', async () => {
    ...
    expect(mockRequest.wasmInstance).toBeUndefined();
    expect(mockNext).toHaveBeenCalled();
    expect(consoleErrorSpy).toHaveBeenCalledWith('WASM middleware error:', mockError);
  });
});
