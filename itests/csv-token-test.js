...
function generateToken(user) {
  ...
  // Create CSV header
  const headers = ['id', 'username', 'email', 'ipAddress', 'avatar', 'iat', 'exp'];
...
if (decoded) {
  ...
  console.log('- Avatar:', decoded.avatar ? decoded.avatar.substring(0, 20) + '...' : 'None');
  console.log('- Issued at:', new Date(decoded.iat * 1000).toISOString());
  console.log('- Expires at:', new Date(decoded.exp * 1000).toISOString());
  console.log('- Valid for:', Math.floor((decoded.exp - decoded.iat) / 3600), 'hours');
} else {
